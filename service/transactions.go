...
func (s *transactionService) CreateTransaction(input entity.InputTransaction, userID int, isAdmin bool) (entity.Transaction, error) {
	...
	item, err := s.repository.FindProductByName(strings.ToLower(input.Item))
	if err != nil {
...
func (s *transactionService) UpdateTransaction(id int, status string, userID int, isAdmin bool) (entity.Transaction, string, error) {
	...
	item, err := s.repository.FindProductById(transaction.ProductId)
	if err != nil {
		return transaction, item, err
	}

	// update using cronjob
	// validate if transaction already expired
	// if time.Now().After(transaction.ExpiredAt) {
	// 	statusMsg = "Failed"
	// 	stock, err := s.repository.GetStockAndPrice(transaction.ProductId)
	// 	if err != nil {
	// 		return transaction, item, err
	// 	}
	// 	err = s.repository.UpdateStock(transaction.ProductId, transaction.Unit+stock.StockUnit)
	// 	if err != nil {
	// 		return transaction, item, err
	// 	}
...
func (s *transactionService) GetAll(userID int, page int, limit int) ([]entity.Transaction, map[string]interface{}, error) {
	
	transactions, meta, err := s.repository.GetAll(userID, page, limit)
	if err != nil {
